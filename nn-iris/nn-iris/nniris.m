...
num_labels = 3;     %4;          % 10 labels, from 1 to 10   
...
sel = randperm(size(iris_x, 1));
sel = sel(1:split);

X=iris_x(sel ,:);
y=iris_y(sel);

X_Val=iris_x((split+1):end,:);
y_val=iris_y((split+1):end);
...
% find(y~=pred);
% errors = ans;
% % % % % % displayData(X(find(y~=pred), :)); 
% % % % % % y(find(y~=pred))
% % % % % % pred(find(y~=pred)) %#ok<NOPTS>
